...
describe("Given that the '/getweather/zipid/countryid' route exists", () => {
  test("When the correct zipid and countryid is provided, API responds with 200 status and weather data", async () => {
  ...
  test("When incorrecrt countryid is provided, API responds with status 400 and an error message ", async () => {
    ...
    const response = await request(app).get("/getweather/400602/INDY").set("Accept", "application/json");
    expect(response.status).toBe(400);
    expect(response.body).toEqual(expectedData);
  });
});
